...
let main _property strategy _seed =
  (* Your logic here: select property and generator by name *)
  let (framework, _generator) = 
  ...
  | "qcheck" ->
      let size x = QCheck2.Test.Int (sizeSTLC x) in
      let test =
        QCheck2.Test.make
          ~name:"SinglePreserve"
          ~count:50000
          ...
          gen_Q_Bespoke
          (fun e ->
            QCheck2.assume (typechecks e);
            Option.value
              (mt e >>= fun t -> Some (mtypeCheck (pstep e) t))
              ~default:true)
      in
      let tyche_arg = [| Sys.argv.(0); "--json" |] in
      let _ = QCheck_tyche.run_tests_main ~argv:tyche_arg  [ test ] in
      exit 0
      (* Runner_qcheck.run
