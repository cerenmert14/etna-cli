...
fn main() {
    ...
    let result = match (tool, property) {
        ("quickcheck", "insert_valid") => {
            qc.quicksample(spec::prop_insert_valid as fn(Tree, i32, i32) -> bool)
        }
        ("quickcheck", "delete_valid") => {
            qc.quicksample(spec::prop_delete_valid as fn(Tree, i32) -> bool)
        }
        ("quickcheck", "union_valid") => {
            qc.quicksample(spec::prop_union_valid as fn(Tree, Tree) -> bool)
        }
        ("quickcheck", "insert_post") => {
            qc.quicksample(spec::prop_insert_post as fn(Tree, i32, i32, i32) -> bool)
        }
        ("quickcheck", "delete_post") => {
            qc.quicksample(spec::prop_delete_post as fn(Tree, i32, i32) -> bool)
        }
        ("quickcheck", "union_post") => {
            qc.quicksample(spec::prop_union_post as fn(Tree, Tree, i32) -> bool)
        }
        ("quickcheck", "insert_model") => {
            qc.quicksample(spec::prop_insert_model as fn(Tree, i32, i32) -> bool)
        }
        ("quickcheck", "delete_model") => {
            qc.quicksample(spec::prop_delete_model as fn(Tree, i32) -> bool)
        }
        ("quickcheck", "union_model") => {
            qc.quicksample(spec::prop_union_model as fn(Tree, Tree) -> bool)
        }
        _ => {
